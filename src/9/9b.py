...
print(len(set(functools.reduce(lambda prev_node, current_node: functools.reduce(lambda old, new: old + [(i[0], i[1]) for i in itertools.zip_longest(range(old[-1][0], new[0], int(math.copysign(1, new[0] - old[-1][0]))), range(old[-1][1], new[1], int(math.copysign(1, new[1] - old[-1][1]))), fillvalue=(new[0] if abs(new[1] - old[-1][1]) > abs(new[0] - old[-1][0]) else new[1]))], prev_node, [(0,0)]), range(0,9), list(itertools.accumulate(open("9.input").readlines(), lambda head_tail, move: (head_tail[0] + (int(move[2:]) if move[0] in "RL" else 0) * (1 if move[0] == "R" else -1), head_tail[1] + (int(move[2:]) if move[0] in "UD" else 0) * (1 if move[0] == "D" else -1)), initial=(0,0)))))))